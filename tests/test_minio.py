...
class TestMinioSetup:
    ...
    def test_create_bucket_creates_when_not_exists(self):
        ...
        create_bucket(client, "orbitharbor-imagery")
        client.make_bucket.assert_called_once_with("orbitharbor-imagery")
    ...
    def test_create_bucket_checks_existence_first(self):
        ...
        create_bucket(client, "orbitharbor-imagery")
        client.bucket_exists.assert_called_once_with("orbitharbor-imagery")
...
class TestTileUploader:
    ...
    def test_upload_tile_calls_fput_object(self, tmp_path):
        ...
        client = MagicMock()
        tile_path = tmp_path / "harbor_watch_20260409_B04.tif"
        tile_path.write_bytes(b"fake tif data")

        upload_tile(client, "orbitharbor-imagery", tile_path)
    ...
    def test_upload_tile_uses_date_prefix(self, tmp_path):
        ...
        key = upload_tile(client, "orbitharbor-imagery", tile_path)
    ...
    def test_upload_tile_returns_object_key(self, tmp_path):
        ...
        client = MagicMock()
        tile_path = tmp_path / "harbor_watch_20260409_B08.tif"
        tile_path.write_bytes(b"fake tif data")

        key = upload_tile(client, "orbitharbor-imagery", tile_path)
        assert "harbor_watch_20260409_B08.tif" in key
