...
class TestImageryPipelineDag:
    """Tests for orbitharbor_imagery_dag.py structure."""

    def setup_method(self):
        self.source = load_dag_source("orbitharbor_imagery_dag.py")

    def test_dag_file_exists(self):
        """orbitharbor_imagery_dag.py exists in dags/."""
        assert Path("dags/orbitharbor_imagery_dag.py").exists()

    def test_dag_id(self):
        """DAG id is imagery_pipeline."""
        assert extract_dag_id(self.source) == "orbitharbor_imagery"
...
class TestAnomalyLoaderDag:
    """Tests for orbitharbor_anomaly_dag.py structure."""

    def setup_method(self):
        self.source = load_dag_source("orbitharbor_anomaly_dag.py")

    def test_dag_file_exists(self):
        """orbitharbor_anomaly_dag.py exists in dags/."""
        assert Path("dags/orbitharbor_anomaly_dag.py").exists()

    def test_dag_id(self):
        """DAG id is anomaly_loader."""
        assert extract_dag_id(self.source) == "orbitharbor_anomaly_loader"
    ...
    def test_has_owner(self):
        ...
        assert "orbitharbor" in self.source
